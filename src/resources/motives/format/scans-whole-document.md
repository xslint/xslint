...
stylesheet is entered at — `match="/"`, with neither `@name` nor `@mode` — is
left alone for the same reason: it is applied to the one document node, so the
walk is paid once there too. Give that template a `@name` and it becomes
callable from anywhere, as often as its callers run. Give it a `@mode` and only
an `xsl:apply-templates` naming that mode reaches it, as often as whatever
holds that instruction runs, and over whatever tree it selects — which need not
be the source document at all, a moded root template applied to a variable's
temporary tree walking that tree instead. Match anything below the root, the
document element included, and `xsl:apply-templates` may reach it more than
once. An `xsl:for-each` inside the root template is the ordinary case again,
its content instantiated once per item.
