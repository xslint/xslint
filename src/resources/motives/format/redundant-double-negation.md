...
Those places are the `@test` of an `xsl:if` or an `xsl:when`, an operand of `and`
or `or`, the argument of another `not()`, the condition of an `if` expression and
the body of a `satisfies`: each takes the effective boolean value of what stands
