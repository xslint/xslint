...
half of #784 answers it again and further: serving a declarative axis from one
shared walk took `xpath-linter` from 55.00% to 39.11%, so the dearest stage is
no longer over half the run, and the nineteen cheap stages came up once more, by
1.209 to 1.465 times what they read before, median 1.376. Three re-derivations
...
moves a third time — `xpath-linter` 39.94%, `corpus-linter` 12.24%,
`xpath-validator` 11.30%, `xsl-validator` 7.42%, taking the dearest of
thirty-six readings — and the three answering to the band are re-derived to 75
for `xpath-linter`, 20 for `xpath-validator` and 12 for `xsl-validator`, which
is 1.88, 1.77 and 1.62 times each reading. The cross-file stage is what makes
doing it deliberately the point: it cost the same to the millisecond and its
reading crossed the 12 it had been given, so leaving the table alone would have
printed a regression against a stage nobody had touched. `SHARE` moves with
them, 5 to 6, and not because a stage crossed it either: the dearest cheap stage
reads 3.48%, and 5 against 3.48% is 1.44, under the band rather than inside it,
where a runner charging a stage the 1.53 the worst of them has charged another
would read 5.3% and turn red on a clean tree. The cross-file entry itself
answers the two-distribution rule and is measured rather than scaled: 20,
between an index reading 7.02% to 12.24% here and the pre-#783 scan reading
21.31% to 27.37% over ten runs of the gate. Both edges of that window move — the
same scan floored at 20.03% before #788 armed the corpus, where 20 would have
let one run through and 19 was the value with room on both sides — which is why
both are taken again on the tree in hand rather than scaled by arithmetic.
...
which the nineteen of them sit below at 0.20% to 3.48% — so a cheap stage that
...
measured distributions**: `corpus-linter` at 20 stands between an index reading
7.02% to 12.24% here and the pre-#783 scan reading 21.31% to 27.37% in the same
tree, the gate failing ten times of ten with that scan in place. Both edges
move, so neither is carried over: before #788 armed the corpus that scan floored
at 20.03% and 20 would have let one run through, where 19 was the value with
room on both sides. #783 drew the entry at 12 by that same rule, twice its
index's dearest 6.05% then, and a **sixth** below the cheapest the scan gave on
any runner, 14.4%; the denominator has moved three times since — at #783,
at #784 twice — so both distributions are taken again on the tree in hand rather
than scaled by arithmetic, which is also how #788 read its index at 8.75%
where #783 read 6.05%, the arming of the corpus accounting for at most a tenth
of that and the day for the rest. Three of those four entries moved without
their stages costing a millisecond more or less, and that is worth knowing
rather than hiding: taking `corpus-linter` from some 16% of the run to some 6%
takes a ninth out of the denominator every share is a share of, and
`xpath-linter`, `xpath-validator` and `xsl-validator` each rose by exactly that
— 1.13, 1.17 and 1.13 against the 1.12 the arithmetic predicts — so their
entries are re-derived by it (75 to 85, 13 to 15, 8 to 9) rather than left to
tighten by a ninth as a side effect of a stage they have nothing to do
with. #784 did it once more and in the other direction, 85 to 75, 15 to 20 and 9
to 12, `xpath-linter` having stopped being over half the run. The entry read 26
while the scan was what the gate had to hold, drawn a tenth above the dearest
reading that scan gave on any runner and a fortieth below the cheapest #755's
quadratic gave here, on CI evidence rather than caution — this is the entry a
runner disagrees about most, four of them charging the scan 14.4%, 19.1%, 22.0%
and 23.5% of the run where this machine charged 15.4%. Everywhere else the
ceiling stands between **half again and twice** the dearest reading, there being
no second distribution to leave room for. The table is a ratchet rather than a
licence, the `SPRAWLING` pattern one property over, and it turns red from both
sides: past the ceiling, or so far under it that `SLACK` (four, for the same
runner's sake) says the ceiling has stopped being a bar and wants retightening.
Growth is asked only of the stages with no entry, since an entry pins what a
stage costs outright, which is the stronger statement; those nineteen read 0.19
to 1.64, against a `GROWTH` bar of 3.0 and the 4.0 a stage that had gone
