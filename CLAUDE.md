...
per document. `corpus-linter` falls from 2.15 s to 0.07 s over DocBook-XSL,
1.16 to 0.06 over TEI and 0.55 to 0.03 over DITA-OT, lowest of three
interleaved rounds a side, which takes the staged run down 34%, 21% and 27% and
...
rest: `xpath-linter` at 64%, `xpath-validator` at 20%, `xsl-validator` at 14%.
Every other stage answers to one bar, `SHARE` at 6%, which the twenty of them
sit far below at 0.27% to 3.80% — so a cheap stage that becomes an expensive one
...
- **Selector hygiene.** A declarative selector must not test existence by
  ...
  only the predicates reach fontoxpath. A wildcard, a root
