...
rest: `xpath-linter` at 50%, `corpus-linter` at 32%, `xpath-validator` at 16%,
`xsl-validator` at 10%. Every other stage answers to one bar, `SHARE` at 5%,
which the nineteen of them sit far below at 0.19% to 3.27% — so a cheap stage
...
measured distributions**: `corpus-linter` at 32 is the **geometric middle** of
the dearest reading #783's index has given here, 20.08%, and the cheapest the
scan it replaced gives over the same corpus, 50.34% — geometric rather than
...
to its pre-#783 state fails the gate three times of three at 50.80%, 51.42% and
50.96%, and the index passes it five of five. Both of those edges move whenever
...
real one has. Three of those four entries moved without their stages costing a
millisecond more or less, and that is worth knowing rather than hiding: taking
...
run now prints is where the reading is read off. Five runs of it on the tree #784
left — one nightly, four dispatched — give 13, 14, 20, 13 and 13 seconds over
DocBook-XSL, 9, 10, 11, 10 and 10 over TEI, and 5, 4, 5, 4 and 3 over DITA-OT,
so the runner disagrees with itself about one tree by half as much again and the
window has to hold a slow night as well as a fast one. The budgets are twice the
dearest of those — **40, 22 and 10** — which leaves the ratchet firing under 10,
5 and 2 seconds, each a reading below the cheapest that corpus has given.
DITA-OT is the one standing near the clock's own edge, three to five seconds
where a tick is a fifth of the reading, so it is the corpus whose ratchet speaks
first and answering it costs one number.
