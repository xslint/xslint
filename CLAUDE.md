...
node rather than `xslint`: it walks a directory of 30,000 files in a process
given a 100kB JavaScript stack, because the crash of #758 needs a spread wider
than the stack carries and scaling the stack down is what puts that within a
...
| `src/xpath.js` | fontoxpath environment: prefixes, evaluators, and `refusalOf`, which since #732 is where `src/grammar.js` becomes the answer a run depends on — `isValid` is that answer with the complaint dropped, for the callers wanting a verdict alone. What the parse knew and a boolean threw away is the offset the fault stood at, so keeping the refusal is what lets a report point at the fault instead of at the attribute holding it (#589); the tree is still not kept, one sentence per distinct expression of a corpus being a different bargain from one tree. It takes the record `expressionsOf` yields rather than a string, because two of the three things it needs are on the node: `matched` judges a `pattern` and `parsed` an expression, and `versionOf` says which language either is read as, so `1 cast as xs:integer` is valid in a 2.0 sheet and a syntax error in a 1.0 one — the whole of #652, and unaskable of an engine that is XPath 3.1 and knows no other dialect. A version `versionOf` cannot place, or none at all, is read at `ASSUMED`, the most permissive version `KNOWN` holds and derived from it rather than spelled: a missing `version` is already `missing-version-in-stylesheet`'s defect, and letting it decide a syntax question would answer one defect with an `invalid-xpath-expression` for every modern expression the file carries. Judging against the specification rather than against fontoxpath takes the engine's own strictness off this path — the `namespace::` axis, and ExprWhitespace around an axis's `::` (#615) or around a node test's brackets (#639) — so the respelling retry those three needed answered nobody's question, and #738 deleted it: 204 lines of regular expressions that rewrote an expression before the engine was asked about it, taking this file from 389 lines to 190. Nothing in `src/` respells anything now. `spelling` in `src/tokens.js` stays, since `opensAxis` asks it whether a name is already in progress (#709) — what went is the retry's use of it, which was the only one from outside that file, so its export went with the retry rather than the question itself. What is left of the pair is `compiles`, the engine as it stands, exported for the two sweeps that diff the grammar against it — a second opinion rather than a verdict, which is what lets it stay strict where the specification is not. The accounting moved with the deletion rather than going away, because the corpus gate was written around what the retry rescued: `insists` in `test/strictness.js` reads the same three spellings off the token stream, so the class is *named* where it used to be *respelled*. Every verdict is kept, keyed by version and language as well as by text, because a corpus asks about `.` and `@name` and `text()` over and over (#689) |
...
| `test/helpers.js` | The only door to a child process in the suite: `runXslint`/`xslintStatus`/`xslintStreams` run the CLI, `xcopped` runs xcop once over a directory, `cmdAvailable` answers whether a tool is there by running it, and `walkedWith(kilobytes, dir)` runs `allFilesFrom` in a process whose JavaScript stack is that small. That last one hands back the largest spread the stack allows beside the walk's own answer, because a walk that survives proves nothing unless the trap was armed and how many arguments a spread carries is V8's business — a Node that moved the number would otherwise leave the test quietly proving nothing (#758) |
