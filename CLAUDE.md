...
| `test/grammar-shapes.test.js` | The same acceptance diff as `grammar-corpus.test.js`, asked of 8064 expressions nobody wrote: every shape a head and one or two tails spell, spaced and glued. A corpus covers only what somebody has already written down, and every class #740 closed stood outside the repository's — so `GAPS` read empty while the grammar refused `text() + 1` and accepted `a?b`, and this sweep parted from the engine 1603 times on the same head. Two classes are left, annotated as classes rather than as their 81 shapes since one cause carries both (#742), and a predicate too broad to be a class is the failure mode: an annotation that swallows the next defect turns nothing red. Half a second, which is why the fast half now answers in under two rather than under one |
